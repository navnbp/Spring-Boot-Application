...
public class DistributionCenterController {
	...
	@RequestMapping(value = "/distribution/all", method = RequestMethod.GET)
	@ResponseBody
	...
	public List<String> simulateSearchResult(String name) {
		...
		if (name == null || name.length() == 0) {
			...
			result = this.distributorList.stream().map(d -> d.getName())
					.filter(d -> d.toLowerCase().contains(name.toLowerCase()))
					.sorted()
					.collect(Collectors.toList());
		}
